...
vmess_ws_habis_status=$(systemctl status vmess-ws-habis | grep running -o)
...
# STATUS SERVICE NON TLS
if [[ $non_tls_status == "running" ]]; then
...
# STATUS VMESS OPOK
if [[ $vmess_ws_opok_status == "running" ]]; then
   sohq1=" ${GREEN}Running ${NC}( No Error )${NC}"
else
   sohq1="${RED}  Not Running ${NC}  ( Error )${NC}"
fi
# STATUS VMESS KUOTA HABIS
if [[ $vmess_ws_habis_status == "running" ]]; then
   sohq2=" ${GREEN}Running ${NC}( No Error )${NC}"
else
   sohq2="${RED}  Not Running ${NC}  ( Error )${NC}"
fi
# STATUS OHP SSH
...
echo -e " * VMESS TSEL      :$sohq1"
echo -e " * VMESS ORBIT     :$sohq2"
